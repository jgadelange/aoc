...
class Parser:
    ...
    def parse(self, g):
        ...
        if t == 4:
            ...
            values = []
            if length_type == "0":
                values.extend(self.parse_total_length(g))
            else:
                values.extend(self.parse_num_packets(g))


    def parse_total_length(self, g):
        l = take_to_int(g, 15)
        print("L", l)
        sub_gen = alt_gen(take_str(g, l))
        try:
            while True:
                yield self.parse(sub_gen)
        except:
            pass

    def parse_num_packets(self, g):
